...
def drop_unimportant_columns(df: pd.DataFrame, col_to_drop: list) -> pd.DataFrame:
    """
...
def preprocess_data(raw_df: pd.DataFrame, scaler_numeric: bool = False, cols_to_drop: list, target_col: str) -> Tuple[pd.DataFrame, pd.Series, pd.DataFrame, pd.Series, List[str], MinMaxScaler, OneHotEncoder]:
    """
